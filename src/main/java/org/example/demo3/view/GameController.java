...
public class GameController implements Initializable {
    ...
    private void subscribeToEvents() {
        eventBus.subscribe(GameStateUpdateEve.class, event -> updateGameState(event));
        eventBus.subscribe(RoundEndedEve.class, event -> displayRoundResult(event));
        eventBus.subscribe(GameEndedEve.class, event -> displayGameOver(event));
    }
    ...
    private VBox createCardUI(Card card) {
        ...
        String powerText = (card instanceof org.example.demo3.model.cards.UnitCard) ? "Power: " + card.getPower() : "Power";
